...
std::optional<common::SettingsValue> InterpretValue(const KeyInfo& key,
    ...
    if (!value && (flags & ArgsManager::DISALLOW_ELISION)) {
        error = strprintf(
            "Can not set -%s with no value. Please specify value with "
            "-%s=value.",
            key.name, key.name);
        return std::nullopt;
    }
...
std::optional<unsigned int> ArgsManager::GetArgFlags(
...
bool ArgsManager::WriteSettingsFile(std::vector<std::string>* errors,
    ...
    if (!GetSettingsPath(&path, /*temp=*/false, backup) ||
        ...
        throw std::logic_error(
            "Attempt to write settings file when dynamic settings are "
            "disabled.");
    }

    LOCK(cs_args);
    std::vector<std::string> write_errors;
    if (!common::WriteSettings(path_tmp, m_settings.rw_settings,
                               write_errors)) {
...
std::optional<std::string> ArgsManager::GetArg(
    ...
    if (cmds != m_available_args.end()) {
        ...
        if (found.size() > 1) {
            throw std::runtime_error(strprintf(
                "Only one of %s may be specified.", util::Join(found, ", ")));
        }
    }
}
    ...
    for (const auto& arg_map : m_available_args) {
        switch (arg_map.first) {
            ...
            case OptionsCategory::IPC:
                usage +=
                    HelpMessageGroup("IPC interprocess connection options:");
                break;
...
fs::path GetDefaultDataDir() {
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Kotucoin
    //   new: C:\Users\Username\AppData\Local\Kotucoin
    // macOS: ~/Library/Application Support/Kotucoin
    // Unix-like: ~/.kotucoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Kotucoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Kotucoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Kotucoin";
#else
    // Unix-like
    return pathRet / ".kotucoin";
#endif
#endif
}
